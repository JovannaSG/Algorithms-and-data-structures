...
// Метода меняет местами два объекта
void swap(Info &obj1, Info &obj2)
{
	Info s = obj1;
	obj1 = obj2;
	obj2 = s;
...
{
	int i = left, j = right;
	//Date pivot = getDate(arrayOfInfo[left]); // Опорный - первый элемент
	Date pivot = arrayOfInfo[left].date;
	...
	do
	{
		...
		if (i <= j)
		{
			...
			{
				swap(arrayOfInfo[i], arrayOfInfo[j]);
			}
			i++;
			j--;
		}
